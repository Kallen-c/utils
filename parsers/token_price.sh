...
while test $# -gt 0; do
	...
	-h|--help)
		...
		echo -e "https://github.com/Kallen-c/utils/blob/main/parsers/token_price.sh - script URL"
		echo -e "https://github.com/Kallen-c/utils/blob/main/databases/tokens.txt - token database"
		echo -e "   "
		echo
...
project=`option_value "$(wget -qO- https://raw.githubusercontent.com/Kallen-c/utils/main/databases/tokens.txt | grep -P "\t${token_symbol^^}\t" | tr -d '\r' | awk -F "\t" '{print $4}')"`
if [ -n "$project" ]; then
	price=`. <(wget -qO- https://raw.githubusercontent.com/Kallen-c/utils/main/parsers/xpath.sh) -x "/html/body/script[1]/text()" -u "https://coinmarketcap.com/currencies/${project}/" | sed "s%<\!\[CDATA\[%%g; s%]]>%%g" | jq ".props.initialProps.pageProps.info.statistics.price"`
else
	price=`. <(wget -qO- https://raw.githubusercontent.com/Kallen-c/utils/main/parsers/xpath.sh) -x "/html/body/script[1]/text()" -u "https://coinmarketcap.com/currencies/${token_symbol}/" | sed "s%<\!\[CDATA\[%%g; s%]]>%%g" | jq ".props.initialProps.pageProps.info.statistics.price"`
fi
if [ -n "$price" ]; then
	if [ -n "$multiplier" ]; then
